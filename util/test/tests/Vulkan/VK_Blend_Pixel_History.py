...
class VK_Blend_Pixel_History(rdtest.TestCase):
    ...
    def primary_test(self):
        ...
        red_eid = self.find_action("Red: groups of repeated draws").next.eventId
        ...
        for i, modif in enumerate(all_modifs):
            ...
            if i < NUM_TRIANGLES_RED:
                ...
                if i != 254:
                    ...
                    if not rdtest.value_compare(modif.postMod.col.floatValue, (NUM_TRIANGLES_RED/255.0, 1.0, 1.0, 1.0), eps=1.0/256.0):
                        raise rdtest.TestFailureException("Wrong post mod for final (blue) triangle in all {}; got {}, wanted {}".format(i, modif.postMod.col.floatValue, (NUM_TRIANGLES_RED/255.0, 1.0, 1.0, 1.0)))
    ...
    def check_modifs_consistent(self, modifs):
        ...
        # With this test's blend configuration, shaderOut and blendSrc should match
        for i in range(len(modifs)):
            if not modifs[i].blendSrc.IsValid():  # Applies to clears
                continue

            a = value_selector(modifs[i].shaderOut.col)
            b = value_selector(modifs[i].blendSrc.col)

            if not rdtest.value_compare(a, b, eps=1.0/256.0):
                raise rdtest.TestFailureException(
                    "shaderOut at {} primitive {}: {} doesn't match blendSrc: {}".format(modifs[i].eventId,
                                                                              modifs[i].primitiveID,
                                                                              a, b))

        # With this test's blend configuration, preMod and blendDst should match
        for i in range(len(modifs)):
            if not modifs[i].blendDst.IsValid():  # Applies to clears
                continue

            a = value_selector(modifs[i].preMod.col)[:3]
            b = value_selector(modifs[i].blendDst.col)[:3]

            if not rdtest.value_compare(a, b, eps=1.0/256.0):
                raise rdtest.TestFailureException(
                    "preMod at {} primitive {}: {} doesn't match blendDst: {}".format(modifs[i].eventId,
                                                                              modifs[i].primitiveID,
                                                                              a, b))

        # With this test's blend configuration, and good driver behavior, blendSrc + blendDst should be about equal to postMod
        for i in range(len(modifs)):
            if not modifs[i].blendDst.IsValid():  # Applies to clears
                continue

            if i == len(modifs) - 1:
                # Final modifs entry, doesn't behave nicely because postMod is wrong
                # This doesn't affect the blue trianges since everything after 255 has blue of 1.0
                continue

            a = value_selector(modifs[i].blendSrc.col)
            b = value_selector(modifs[i].blendDst.col)
            sum_a_b = [sum(x) for x in zip(a, b)]
            c = value_selector(modifs[i].postMod.col)

            if not rdtest.value_compare(sum_a_b, c, eps=1.0/256.0):
                raise rdtest.TestFailureException(
                    "blendSrc at {} primitive {}: {} plus blendDst: {} (sum: {}) should equal postMod: {}".format(modifs[i].eventId,
                                                                              modifs[i].primitiveID,
                                                                              a, b, sum_a_b, c))
